...
def create_organization_entities(result):
...
def create_location_entities(result):
    fhirID=result.get('id')
...
def create_patient_entities(result): 
...
def create_encounter_entities(result):
...
def create_procedure_entities(result):
...
def create_condition_entities(result):
    fhirID = result.get('id')
    identifier = get_identifier(result.get('identifier', []))
...
def create_medicationDispense_entities(result):
...
def create_medicationRequest_entities(result):
...
def create_specimen_entities(result):
...
def create_medication_entities(result):
...
def create_medicationAdministration_entities(result):
    def get_category(cat):
        if len(cat)==0:
            return ""
        return f"""\t\t\tfhir:category [
...
def create_observation_entities(result):
    def get_category(cat):
        return f"""\t\t\tfhir:category [
                fhir:coding [
...
def convert(pc,output):
    x=""
    if pc == "Patient":
        x=create_patient_entities(output)
    elif pc == "Observation":
        x=create_observation_entities(output)
    elif pc == "MedicationAdministration":
        x=create_medicationAdministration_entities(output)
    elif pc == "Medication":
        x=create_medication_entities(output)
    elif pc == "MedicationRequest":
        x=create_medicationRequest_entities(output)
    elif pc == "MedicationDispense":
        x=create_medicationDispense_entities(output)
    elif pc == "Specimen":
        x=create_specimen_entities(output)
    elif pc == "Condition":
        x=create_condition_entities(output)
    elif pc == "Procedure":
        x=create_procedure_entities(output)
    elif pc == "Encounter":
        x=create_encounter_entities(output)
    elif pc == "Location":
        x=create_location_entities(output)
    elif pc == "Organization":
        x=create_organization_entities(output)
    if x==None:
        return ""
    return x
    
def pipeline(pc,pl):
    query = {"resourceType": pc}
    #for x in pl:
    #    query[x] = {"$exists": True}
    result = collection.find_one(query)
    return convert(pc, result)

def get_sample(props):  
    prop_class=""
    final_sample=""
    props_list=[]
    word=""
    for x in props:
        if x == '[':
            ...
            props_list.append(word)
            word=""
        elif x==']':
            props_list.append(word)
            final_sample=final_sample+pipeline(prop_class,props_list)
            prop_class=""
            props_list=[]
            word=""
        else:
